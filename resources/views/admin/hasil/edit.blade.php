            ...
            <form action="{{ route('admin.hasil.sempro.update', $hasil->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <select id="jadwal_sempro_id" name="jadwal_sempro_id"
                        ...
                        @foreach ($jadwalSemproList as $jadwal)
                            <option value="{{ $jadwal->id }}"
                                {{ old('jadwal_sempro_id', $hasil->jadwal_sempro_id) == $jadwal->id ? 'selected' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" id="nilai_peng1" name="nilai_peng1" step="0.01" min="0" max="100"
                        value="{{ old('nilai_peng1', $hasil->nilai_peng1) }}"
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500">
                    @error('nilai_peng1')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <input type="number" id="nilai_peng2" name="nilai_peng2" step="0.01" min="0" max="100"
                        value="{{ old('nilai_peng2', $hasil->nilai_peng2) }}"
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500">
                    @error('nilai_peng2')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <input type="number" id="nilai_peng3" name="nilai_peng3" step="0.01" min="0" max="100"
                        value="{{ old('nilai_peng3', $hasil->nilai_peng3) }}"
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500">
                    @error('nilai_peng3')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <select id="status" name="status"
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500">
                        <option value="lolos_tanpa_revisi"
                            {{ old('status', $hasil->status) == 'lolos_tanpa_revisi' ? 'selected' : '' }}>Lolos Tanpa
                            Revisi</option>
                        <option value="revisi_minor"
                            {{ old('status', $hasil->status) == 'revisi_minor' ? 'selected' : '' }}>Revisi Minor</option>
                        <option value="revisi_mayor"
                            {{ old('status', $hasil->status) == 'revisi_mayor' ? 'selected' : '' }}>Revisi Mayor</option>
                        <option value="tidak_lolos" {{ old('status', $hasil->status) == 'tidak_lolos' ? 'selected' : '' }}>
                            Tidak Lolos</option>
                    </select>
                    @error('status')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                ...
                <div class="mb-4">
                    ...
                    @if ($hasil->revisi_file_path)
                        <p class="text-sm text-gray-600 mt-1">File saat ini: <a
                                href="{{ Storage::url($hasil->revisi_file_path) }}" target="_blank"
