...
@section('title', 'Tambah Pengajuan Sempro')

@section('content')
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow-md">
            <div class="flex items-center justify-between mb-6">
                <h2 class="text-2xl font-semibold text-gray-800">Tambah Pengajuan Sempro</h2>
            ...
            <form action="{{ route('admin.pengajuan-sempro.store') }}" method="POST">
                @csrf
                <div class="mb-4">
                    ...
                    <select id="mahasiswa_id" name="mahasiswa_id"
                        ...
                        @foreach ($mahasiswa as $mhs)
                            <option value="{{ $mhs->id }}">{{ $mhs->user->name }} (NIM: {{ $mhs->nim }})</option>
                        @endforeach
                    </select>
                    @error('mahasiswa_id')
                ...
                <div class="mb-4">
                    ...
                    <textarea id="abstrak" name="abstrak"
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500">{{ old('abstrak') }}</textarea>
                    @error('abstrak')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select id="dosen_pembimbing_id" name="dosen_pembimbing_id"
                        ...
                        @foreach ($dosen as $dsn)
                            <option value="{{ $dsn->id }}">{{ $dsn->user->name }} (KK: {{ $dsn->bidangKeilmuan->name }})</option>
                        @endforeach
                    </select>
