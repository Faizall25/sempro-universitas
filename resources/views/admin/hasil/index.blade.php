...
<div class="container mx-auto px-4 py-8">
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route('admin.hasil.sempro.create') }}" class="inline-block px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition duration-300 ease-in-out" data-tooltip="Tambah hasil sempro baru">
                ...
                <tbody>
                    @foreach ($hasil as $index => $item)
                        <tr class="border-b border-gray-200 hover:bg-gray-100 transition duration-200 ease-in-out">
                            ...
                            <td class="px-6 py-4 text-gray-700">{{ $item->jadwalSempro->pengajuanSempro->judul ?? 'N/A' }}</td>
                            ...
                            <td class="px-6 py-4 text-gray-700">
                                <span class="inline-block px-2 py-1 text-sm font-semibold rounded-full {{ $item->status == 'lolos_tanpa_revisi' ? 'bg-green-100 text-green-800' : ($item->status == 'revisi_minor' ? 'bg-yellow-100 text-yellow-800' : ($item->status == 'revisi_mayor' ? 'bg-orange-100 text-orange-800' : 'bg-red-100 text-red-800')) }}">
                                    {{ str_replace('_', ' ', ucwords($item->status)) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-gray-700">
                                @if ($item->revisi_file_path)
                                    <a href="{{ Storage::url($item->revisi_file_path) }}" target="_blank" class="text-blue-600 hover:underline">Lihat File</a>
                                @else
                                    Tidak ada
                                @endif
                            </td>
                            <td class="px-6 py-4 flex space-x-2">
                                <a href="{{ route('admin.hasil.sempro.edit', $item->id) }}" class="relative px-3 py-1 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition duration-300 ease-in-out" data-tooltip="Edit hasil">
                                    <i class="fas fa-edit"></i>
                                    <span class="tooltip">Edit</span>
                                </a>
                                <form action="{{ route('admin.hasil.sempro.destroy', $item->id) }}" method="POST" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="relative px-3 py-1 bg-red-500 text-white rounded-lg hover:bg-red-600 transition duration-300 ease-in-out" data-tooltip="Hapus hasil" onclick="return confirm('Apakah Anda yakin ingin menghapus hasil ini?')">
