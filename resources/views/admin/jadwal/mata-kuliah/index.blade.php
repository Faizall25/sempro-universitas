...
<div class="container mx-auto">
    ...
    <a href="{{ route('admin.jadwal.mata-kuliah.create') }}" class="inline-block px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 mb-4 transition duration-300 ease-in-out" data-tooltip="Tambah jadwal mata kuliah baru">
                    ...
                    @foreach ($jadwal as $index => $item)
                        <tr class="border-b border-gray-200 hover:bg-gray-100 transition duration-200 ease-in-out">
                            ...
                            <td class="px-6 py-4 flex space-x-2">
                                <a href="{{ route('admin.jadwal.mata-kuliah.edit', $item->id) }}" class="px-3 py-1 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition duration-300 ease-in-out" data-tooltip="Edit jadwal">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="{{ route('admin.jadwal.mata-kuliah.destroy', $item->id) }}" method="POST" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="px-3 py-1 bg-red-500 text-white rounded-lg hover:bg-red-600 transition duration-300 ease-in-out" data-tooltip="Hapus jadwal" onclick="return confirm('Apakah Anda yakin ingin menghapus jadwal ini?')">
