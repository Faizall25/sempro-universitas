    ...
    <div class="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow-md">
        ...
        <form action="{{ route('admin.jadwal.sempro.update', $jadwal->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select id="pengajuan_sempro_id" name="pengajuan_sempro_id" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500">
                    ...
                    @foreach ($pengajuanSemproList as $pengajuan)
                        <option value="{{ $pengajuan->id }}" {{ old('pengajuan_sempro_id', $jadwal->pengajuan_sempro_id) == $pengajuan->id ? 'selected' : '' }}>{{ $pengajuan->judul }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <input type="date" id="tanggal" name="tanggal" value="{{ old('tanggal', $jadwal->tanggal->format('Y-m-d')) }}" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500">
                @error('tanggal')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="time" id="waktu" name="waktu" value="{{ old('waktu', $jadwal->waktu->format('H:i')) }}" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500">
                @error('waktu')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="dosen_penguji_1" name="dosen_penguji_1" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500">
                    ...
                    @foreach ($dosenList as $dosen)
                        <option value="{{ $dosen->id }}" {{ old('dosen_penguji_1', $jadwal->dosen_penguji_1) == $dosen->id ? 'selected' : '' }}>{{ $dosen->user->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <select id="dosen_penguji_2" name="dosen_penguji_2" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500">
                    <option value="">-- Pilih Dosen --</option>
                    @foreach ($dosenList as $dosen)
                        <option value="{{ $dosen->id }}" {{ old('dosen_penguji_2', $jadwal->dosen_penguji_2) == $dosen->id ? 'selected' : '' }}>{{ $dosen->user->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <select id="dosen_penguji_3" name="dosen_penguji_3" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500">
                    <option value="">-- Pilih Dosen --</option>
                    @foreach ($dosenList as $dosen)
                        <option value="{{ $dosen->id }}" {{ old('dosen_penguji_3', $jadwal->dosen_penguji_3) == $dosen->id ? 'selected' : '' }}>{{ $dosen->user->name }}</option>
                    @endforeach
                </select>
